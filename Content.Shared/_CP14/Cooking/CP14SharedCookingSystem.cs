...
public abstract partial class CP14SharedCookingSystem : EntitySystem
{
    ...
    [Dependency] protected readonly SharedUserInterfaceSystem _ui = default!; // STELLAR
    [Dependency] private readonly SharedAudioSystem _audio = default!; // STELLAR
    ...
    /// </summary>
    private readonly ProtoId<ReagentPrototype> _burntFoodReagent = "StellarBurntFood";
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<CP14CookerRelayComponent, PowerChangedEvent>(OnPowerChanged); // BEGIN STELLAR
        SubscribeLocalEvent<CP14CookerRelayComponent, BoundUIClosedEvent>(OnClosed);
        SubscribeLocalEvent<CP14CookerRelayComponent, BoundUIOpenedEvent>(OnOpened); // END STELLAR
    }
    ...
    private void OnPowerChanged(Entity<CP14CookerRelayComponent> ent, ref PowerChangedEvent args)
    {
    ...
    private void OnClosed(Entity<CP14CookerRelayComponent> ent, ref BoundUIClosedEvent args)
    {
        ent.Comp.Closed = true;
        Dirty(ent);

        if (!_ui.IsUiOpen(ent.Owner, args.UiKey) && ent.Comp.Powered)
            RelayCooking(ent);
    }

    private void OnOpened(Entity<CP14CookerRelayComponent> ent, ref BoundUIOpenedEvent args)
    {
    ...
    protected void RelayCooking(Entity<CP14CookerRelayComponent> ent)
    {
        if (!TryComp<StorageComponent>(ent, out var container))
            return;

        foreach (var item in container.StoredItems)
        {
            if (!TryComp<CP14FoodCookerComponent>(item.Key, out var cooker))
                continue;
            if (!TryComp<CP14FoodHolderComponent>(item.Key, out var holder))
                continue;

            if (ent.Comp.Powered && ent.Comp.Closed)
            {
                if (!_doAfter.IsRunning(cooker.DoAfterId) && holder.FoodData is null)
                {
                    var recipe = GetRecipe((item.Key, cooker));
                    if (recipe is not null)
                        StartCooking((item.Key, cooker), recipe);
                }
                else
                {
                    if (cooker.CanBurn)
                        StartBurning((item.Key, cooker));
                }
            }
            else
            {
                StopCooking((item.Key, cooker), ent);
            }
        }
    }
    ...
    private void OnExaminedEvent(Entity<CP14FoodHolderComponent> ent, ref ExaminedEvent args)
    {
        ...
        args.PushMarkup(Loc.GetString("stellar-cooking-examine",
        ...
        Entity<CP14FoodHolderComponent> source)
    {
        ...
        if (_solution.TryGetSolution(target.Owner, holderFoodComp.Solution, out var holderSoln, out var solution))
        {
            if (solution.Volume > 0)
            {
                _popup.PopupEntity(Loc.GetString("stellar-cooking-popup-not-empty", ("name", MetaData(target).EntityName)),
                    target);
                return false;
            }
    ...
    // BEGIN - STELLAR STATION CHANGES (AftrLite)
    protected void CreateSpecificResults(Entity<CP14FoodCookerComponent> ent, CP14CookingRecipePrototype recipe)
    {
        if (!_container.TryGetContainer(ent, ent.Comp.ContainerId, out var container))
            return;
        if (!_solution.TryGetSolution(ent.Owner, ent.Comp.SolutionId, out var soln, out var solution))
            return;
        if (recipe.SpecificResults is null)
            return;
        ...
        //Spawn recipe result
        foreach (var result in recipe.SpecificResults)
        {
            Spawn(result, ent.Owner.ToCoordinates());
        }
    ...
    protected void BurntFood(Entity<CP14FoodCookerComponent> ent)
    {
        ...
        {
            visuals.Color = Color.FromHex("#535353ff");
        }

        holder.FoodData.Name = Loc.GetString("stellar-recipe-meal-burned-trash-name");
        holder.FoodData.Desc = Loc.GetString("stellar-recipe-meal-burned-trash-desc");
